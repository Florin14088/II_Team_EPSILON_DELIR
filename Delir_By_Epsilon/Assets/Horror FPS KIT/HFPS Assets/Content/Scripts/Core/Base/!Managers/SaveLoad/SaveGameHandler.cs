...
public class SaveGameHandler : Singleton<SaveGameHandler> {
    ...
    public void SaveSerializedData(bool allData)
    {
        ...
        if (allData)
        {
            ...
            playerData.Add("cameraRotation", player.GetComponentInChildren<MouseLook_HFPS>().GetRotation());
    ...
    void LoadSerializedData(bool loadWhole)
    {
        if (loadWhole)
        {
            ...
            player.GetComponentInChildren<MouseLook_HFPS>().SetRotation(rotToken.ToObject<Vector2>());
